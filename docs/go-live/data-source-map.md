...
| Siste strukturerte regnskapsår og hovedtall | Brreg Regnskapsregisteret, strukturert JSON | `BrregFinancialsProvider.fetchStructuredAnnualAccounts` → versjonert mapper → `structured-financials-service` → `FinancialStatement` → offentlig kildeport | «Ikke tilgjengelig» for manglende filing eller ikke støttet oppstillingsplan; siste offisielle snapshot merkes utdatert ved midlertidig kildefeil | Provider, mapper, persistence, read-through/cache, proveniens og offentlig anti-fallback er implementert. Første reelle K0-måling finnes i [Sprint 2-rapporten](./sprint-2/structured-financial-coverage.md); bredere dekning gjenstår |
