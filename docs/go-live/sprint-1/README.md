...
| GL-104 | Inputvalidering | Pågår | Simen Lippestad | Første maskinelle delport inventerer 80 muterende API-rutefiler og krever valideringsmarkører for 60 request-body-, 50 dynamiske path- og 12 query-string-flater. 47 dynamiske flater bruker felles ID-filter, tre har lokale Zod-skjema, og scheduler-grensen er 0–100. Profilsøk og underenhetssøk har GET-atferdstester. Direkte organisasjonsnummerfelt bruker nå én felles kontrakt som normaliserer mellomrom og validerer ni sifre med MOD11-kontrollsiffer på tvers av 23 API-rutefiler, profiloppdatering, workspace-actions og fire AI-verktøykontrakter. Resterende GET-only-inventar gjenstår før lukking. |
...
- fortsett GET-only-gjennomgangen utover de testede profil-, underenhets- og årsrapportendepunktene, og lukk GL-104 først når alle kritiske ruter har atferdstester;
