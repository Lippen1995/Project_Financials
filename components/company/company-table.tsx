...
export function CompanyTable({ companies }: { companies: NormalizedCompany[] }) {
  if (companies.length === 0) {
    return (
      <div className="rounded-[1.75rem] border border-dashed border-ink/15 bg-white/70 p-8 text-center text-sm text-ink/60">
      ...
      <table className="min-w-full divide-y divide-ink/10 text-sm">
        <thead className="bg-sand">
          <tr className="text-left text-ink/60">
            ...
            <th className="px-5 py-4 font-medium">Registrert</th>
          </tr>
        </thead>
            ...
            <tr key={company.orgNumber} className="hover:bg-sand/50">
              <td className="px-5 py-4">
                <Link
                  ...
                  className="font-semibold text-tide hover:text-ember"
                >
                  {company.name}
                </Link>
