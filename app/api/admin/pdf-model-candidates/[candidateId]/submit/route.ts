...
export async function POST(
  ...
  try {
    ...
    const data = await submitPdfModelCandidateForReview(candidateId, user.id, parsed.data.note);
    return NextResponse.json({ data });
  } catch (err) {
    if (err instanceof PdfModelCandidateError) {
      return NextResponse.json({ error: err.message }, { status: err.statusCode });
    }
    return NextResponse.json({ error: "Could not submit candidate." }, { status: 400 });
  }
}
