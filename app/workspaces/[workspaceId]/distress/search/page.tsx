...
function buildPageHref(
  ...
  return `/workspaces/${workspaceId}/distress/search?${params.toString()}` as Route;
}
...
function buildResetHref(workspaceId: string): Route {
  return `/workspaces/${workspaceId}/distress/search` as Route;
}

function buildExportHref(rows: Awaited<ReturnType<typeof listDistressCompaniesForWorkspace>>["items"]) {
  const header = [
    "orgNumber",
    "companyName",
    "status",
  ...
  const lines = rows.map((row) =>
    [
      row.company.orgNumber,
      row.company.name,
      row.distress.status,
      row.distress.daysInStatus ?? "",
      row.company.industryCode?.code ?? "",
      row.sector?.code ?? "",
      row.financials.lastReportedYear ?? "",
      row.financials.revenue ?? "",
      row.financials.ebit ?? "",
      row.financials.netIncome ?? "",
      row.financials.equityRatio ?? "",
      row.financials.assets ?? "",
      row.financials.interestBearingDebt ?? "",
    ]
      .map(escapeCsv)
      .join(","),
  );
...
export default async function DistressWorkspacePage({
  ...
  const activeFilterEntries = Object.entries(query).filter(([key, value]) => {
    if (key === "page" || key === "size" || key === "sort" || key === "view") {
      return false;
    }
      ...
      <section className="grid gap-0 border border-[rgba(15,23,42,0.08)] bg-[rgba(255,255,255,0.78)] xl:grid-cols-[minmax(0,1.45fr),360px]">
        <div className="p-8">
          ...
          <h1 className="editorial-display mt-5 max-w-5xl text-[3rem] leading-[0.98] text-slate-950 sm:text-[4rem]">
            Screener for distress-selskaper i {workspaceHome.currentWorkspace.name}.
          </h1>
          <p className="mt-4 max-w-3xl text-[1.02rem] leading-8 text-slate-600">
            Bruk filtrene for å prioritere distress-kandidater etter status, varighet, sektor og siste tilgjengelige regnskapssignal.
          </p>
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[rgba(15,23,42,0.08)] pb-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {activeFilterEntries.map(([key, value]) => (
                    <span
                      ...
                      className="rounded-full border border-[rgba(15,23,42,0.1)] bg-white px-3 py-1 text-xs text-slate-600"
                    >
                      {key}: {Array.isArray(value) ? value.join(", ") : value}
                    </span>
                  ))}
                  <Link href={buildResetHref(workspaceId)} className="text-xs font-semibold text-slate-700 hover:text-slate-950">
                    Nullstill filtre
                  </Link>
                </div>
              ) : null}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <a
                href={buildExportHref(screening.items)}
                download={`distress-screening-${workspaceId}-page-${screening.page + 1}.csv`}
                className="rounded-full border border-[rgba(15,23,42,0.1)] bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-[rgba(15,23,42,0.18)] hover:text-slate-950"
              >
                Last ned CSV
              </a>
