...
export function createFinancialsRepository(
  ...
  return {
    ...
    async listLatestReportedCompanyMetrics() {
      if (!dataSource.readLatestReportedCompanyMetrics) {
        throw new Error("Latest reported company metrics are unavailable.");
      }
...
function companyMapPublishedFinancialsCte(
  query: Pick<CompanyMapFinancialQuery, "buildId" | "statementScope" | "currency">,
) {
  return Prisma.sql`
...
async function readCompanyMapBuildFinancialVersion(
...
const prismaLiveFinancialsDataSource: LiveFinancialsDataSource = {
  async readLatestReportedCompanyMetrics() {
    // This is a privileged publication-job read, not an interactive runtime read. It deliberately
    // fixes the session to reported-only before projecting the complete map snapshot.
          ...
          transaction.$queryRaw<LatestReportedCompanyMetrics[]>(Prisma.sql`
            WITH ranked_statements AS (
              ...
              SELECT
                ...
                CASE
                  WHEN count(*) FILTER (WHERE line."metricKey" = 'profit_before_tax') = 1
    ...
    return prisma.$transaction(
      async (transaction) => {
        ...
        if (remaining > 0) {
          ...
          pageRows.push(
            ...(await transaction.$queryRaw<CompanyMapFinancialRankingDatabaseRow[]>(Prisma.sql`
              WITH ${ctes}
              SELECT
                entity."orgNumber",
                entity."name",
        ...
        const parsedFinancials = parseLatestReportedCompanyMetricsSnapshot({
          financialDatasetVersion,
          statements: pageRows
            .filter((row) => row.companyId !== null && row.companyId !== undefined)
            .map((row) => ({
        ...
        const financialByOrgNumber = new Map(
          parsedFinancials.statements.map((financial) => [financial.orgNumber, financial]),
        );
